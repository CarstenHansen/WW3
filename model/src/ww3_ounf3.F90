...
!/    28-Feb-2022 : MFIT+XSTO options: Stokes drift     ( version 7.XX )
...
!
      USE W3OUNF3METAMD, ONLY: INIT_META, TEARDOWN_META, GETMETA,       &
                              ...
                              NFSMAX
#ifdef W3_MFIT
      USE W3ODATMD, ONLY : IXSP, JXSP, NOEXTR, NOXSM 
      ! (Will also call W3IOGOMD: W3FLDTOIJ('XSP   ', IXSP, JXSP, 1, 1, 1))
#endif
!
      USE NETCDF
...
! 4.2 Output fields
      ...
      END DO
#ifdef W3_MFIT
      ! Write UXSP(1:3) to netCDF if not the Stokes profile is selected
      IF ( FLG2D(10,NOEXTR+1) .AND. .NOT. FLG2D(IXSP,JXSP) ) THEN
        SVP=.FALSE.
        FLG2D(IXSP,JXSP) = .TRUE.
      END IF
#endif
...
# ifdef W3_MFIT
          IF ( FLG2D(10,NOEXTR+1) ) THEN
            ! Multiply by 2*ksc
            UXSP(1:NSEA,2) = UXSP(1:NSEA,2)*UXSP(1:NSEA,1)*2.
            UXSP(1:NSEA,3) = UXSP(1:NSEA,3)*UXSP(1:NSEA,1)*2.
            ...
            where ( UXSP(:,2) * UXSP(:,2) + UXSP(:,3) * UXSP(:,3) < 1.09e-9 )
              UXSP(:,2) = 2.34e-5
              UXSP(:,3) = 2.34e-5
              end where
            M2K(:) = sqrt(UXSP(:,2) * UXSP(:,2) + UXSP(:,3) * UXSP(:,3) )
...
!
            ...
            ELSE IF ( IFI .EQ. 10 .AND. IFJ .EQ. NOEXTR+1 ) THEN
              ...
              IF ( IFS .EQ. 1 ) THEN
                ...
                CALL S2GRID(UXSP(:,4), XX)
                CALL S2GRID(UXSP(:,4+NZO), XY)
              ELSE IF ( IFS .EQ. 2 ) THEN
                NFIELD=2
