...
!/    02-Jan-2023 : Add option USXT = 'Pf-5': Frequency^{-5} tail - no further
!/                  broadening as function of frequency
!-------------------
!>
...
!>  -                        'Pf-5': Frequency^{-5} tail - no further broadening
...
!>  rho g M(ft), where
!>  @verbatim
!>    M(ft) = \int_ft^\inf int_-pi^pi kv/sig H(sig,theta) d theta d sig
!>  @endverbatim
!>  Here, ft it the diagnostic tail start frequency, and kv is the wave number
!>  vector, kv = k * (cos(theta), sin(theta)).  M is a first 'kv/sig'-moment
!>  - like the the first spectral moment, but considered a vector sum.
!>  Assume the tail components are deep-water waves, only, k = sig^2/g, and
!>  assume the Stokes mean direction theta0 is constant over the diagnostic
!>  tail. Then we will formally write
!>  @verbatim
!>    M(ft) = g Dsn fp^-2 Im(nt), nt = log(ft/fp)/log(DFH)
!>  @endverbatim
!>  where Dsn is the prognostic spectral level at ft, and fp is a formal
!>  'peak frequency' value that makes the tail extension match the prognostic
!>  spectrum at ft.
!>
...
!>       Handle namelists parameters read in as:
!>  @verbatim
!>         &STVP NDP = 11, DSC = 2.0, BP = 2.0 /
!>         &XSTP USXT = 'DoEw' , USXF = 2.0 /
!>  @endverbatim
...
!/ ------------------------------------------------------------------- /
      USE CONSTANTS, ONLY: GRAV, TPI, RADE
!/
...
!      diagnostic Stokes array, and log of the increment frequency factor
      REAL               :: DFH, XKH, logdf
...
! ftr: truncation frequency, default 2 Hz
...
!
      ...
      ! If the spectral tail is truncated at a specifified frequency USXF,
      ! IF ( USXF < SIG(NK)/TPI) THEN
      !     IF ( stvp_verbose .gt. 0 ) &
...
! cha 20121123: Shift fitting interval limits away from NK, from NK-4 to NK-1
      IF ( USXT == 'DoEw' .OR. USXT == 'Pf-5' ) THEN
         ! Donelan-Ewans or Phillips saturation
         IKT = NK - 1
...
! array for the diagnostic part of the Stokes profile
      NP = NK
      XKH = XFR**2
! DFH: Relative frequency increment
      DFH = XFR
! Log of the increment frequency factor                             
      logdf = log(DFH) ! Be aware: DFH may *differ* from the prognostic bin
                       ! increment factor XFR

      ! The truncation frequency USXF corresponds to a bin number ntr, where
      ! USXF/fp=DFH**ntr, or ntr = log( USXF/fp ) / log(DFH).
      ! Relative to the transition frequency fd = SIG(IKT) / TPI we have
      ! USXF/fd = (USXF/fp) / (fd/fp). The tail truncation bin exceeds the
      ! transition bin at fd by ntre bins, where USXF/fd = DFH**ntre, or:
      IF (USXF >= 10.0) then
         ntre = NP ! Default 10.0 Hz understood as infinity
      else
         ntre = log( USXF * TPI / SIG(IKT) ) / logdf
      end if
      ...
      allocate( SBDiaZ(SPND, NP), stat=IERR )

      ! Im(bin): A look-up table for integrated momemtum
      allocate ( Im(NP+1), stat=IERR )

      ! Stokes drift for the diagnostic tail.
      IF ( USXT == 'DoEw' ) THEN
        ! Calculate a normalized array DStkOsn(1:NP)
      ...
      DO nt=1,NP
        ...
        SBDiaZ(:,nt) = DStkOsn(nt) * exp( -tRk * ZK_S(:) )
      END DO
...
! Integral pseudo-momentum parameters
      REAL                  :: A2M, Mdiag, MtransX, MtransY
...
! Stokes drift wave number scale and parameters for 'inlined version of WAVNU1'
      ...
      ELSE
        ...
        ! At the highest frequencies of the prognostic range, derive the mean
        ! direction thetat, the 1st circular moment m1t, and the spectral level
        ! Dsn
        call prog_edge( A, Pn, thetat, m1t, Dsn, JSEA)
        ...
        IF ( fOfp(JSEA) .eq. 0.0 ) THEN
          ...
          fOfp_ini = max( 1.2 * SIG(IKT) / SIG_S, fOfp_min )
          fOfp(JSEA) = fOfp_ini
          IF ( stvp_verbose .gt. 2 ) &
        ...
        IF ( m1t .ne. 0 ) THEN
          ...
          IF ( II > 0 ) THEN
            ...
            fOfp(JSEA) = max( 1.2 * SIG(IKT) / SIG_S, fOfp_min )
            ...
            IF ( II > 0 ) THEN
               ...
               IF ( stvp_verbose .gt. 0 ) THEN
                  WRITE (NDSV, 900),                                &
                       'Issue in w3stvpmd, equivalent_tail() for SIG_S = ', SIG_S
        ...
        IF ( stvp_verbose .gt. 1 ) THEN
          WRITE (NDSV, 905), 'ISEA =', ISEA, ' Dsn/U10 =', Dsn/U10(JSEA), &
               ' U10 =', U10(JSEA), ' SIG_S =', SIG_S
          WRITE (NDSV, 905), &
               ' ft/fP =', fOfp(JSEA), ' StkDiag0 =', StkDiag(1)*Dsn, &
               ' Mdiag = ', Mdiag
        END IF

        ! Multiply with the normalization factor
        StkDiag(:) = StkDiag(:) * Dsn
      ...
      IF ( IKT >= IKST ) THEN
        ...
        U_S(:) = U_S(:) + StkDiag(:) * CTH
        V_S(:) = V_S(:) + StkDiag(:) * STH
        M_X = M_X + MtransX + Mdiag * CTH
        M_Y = M_Y + MtransY + Mdiag * STH
      END IF
...
!
  ...
  915 FORMAT ('  ',A,9F8.4)
      ...
      !  m1 ~ m1E(f/fP), based on at fit to observations (the 'Maui'
      !  experiment).
      !
      ...
      !     m1 ~ m1E = cos(theta_m1) * exp( -(sigma**2)/2 )
      !
      !  In the vicinity of fh=f/fP ~ fh0 ~ 2.5, this may be approximated as
      !     m1 ~ m1t(fh) = m1E(fh=1) * exp( aa sinh(phih * (fh - 1.)) )
      !  where
      !     phih = 1/(aa m1) * {d m1 / d fh}
      !  The approximation m1 ~ m1t(fh) converges to zero at high frequences.
      !
...
!/
      ...
      ! The bin number associated with fOfpCut is, with logdf = log(DFH),
...
!/
      ...
      SUBROUTINE prog_edge(A, Pn, theta, m1, alphaU, JSEA)

! Level of the spectral tail
! At the prognostic edge where the tail is to be matched, estimate:
!        theta:   The mean direction
!        m1:      The 1st circular moment m1
!        alphaU:  The spectral level (alpha * U for Pn=4, or B * GRAV for Pn=5)
!
...
!/
      ...
      REAL, INTENT(IN)        :: Pn ! Assumed neg. power of spectral decay (4/5)
      REAL, INTENT(OUT)       :: theta, m1, alphaU
      INTEGER, INTENT(IN)     :: JSEA

      REAL, allocatable, save :: DTHsigPn_CgG(:,:)
      REAL, allocatable       :: DTHsigPn_G(:)
...
! Spectral level of the tail. Assuming S(sig) = alphaU * GRAV * sig^-Pn:
! alphaU = band_mean( \sum A(IK) * FACTOR*SIG(IK)*Pn/GRAV ), where
...
! If it is a saturation range, Pn = 5 and alphaU represents B*GRAV (Phillips'
! parameter B times gravity).

! Spectral factor for calculating the spectral level of the tail over an
! interval [IKST,IKT] of bands near NK
      IF ( .not. allocated(DTHsigPn_CgG) ) THEN
        allocate ( DTHsigPn_CgG(NK,NSEAL), stat=IERR )
        allocate ( DTHsigPn_G(NK), stat=IERR )

        DO IK=1,NK
          DTHsigPn_G(IK) = DTH * SIG(IK)**(Pn+1)/GRAV
        END DO
...
#endif
          DO IK=1,NK
            DTHsigPn_CgG(IK,KSEA) = DTHsigPn_G(IK) / CG(IK,ISEA)
          END DO

        END DO

          deallocate ( DTHsigPn_G )
      END IF
...
! in w3iogomd)
      ...
      ! Spectral level and circular moment from the average over the
      ! interval [IKST,IKT]
      ...
      DO IK = IKST,IKT
        ...
        SUM  = SUM  + ABst * DTHsigPn_CgG(IK,JSEA)
        SUMX = SUMX + ABsX * DTHsigPn_CgG(IK,JSEA)
        SUMY = SUMY + ABsY * DTHsigPn_CgG(IK,JSEA)
      END DO
