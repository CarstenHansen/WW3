...
!
     ...
     ELSE IF ( IFI .EQ. IXSP .AND. IFJ .EQ. JXSP ) THEN
        ! The three subfields (NSP=3) are joined in the netCDF file
        NSP = 3 ! Number of subfields loop over ISP=1,NSP
        ISUB = ISP
        FLJOIN = .TRUE.
!
        IF ( ISP .EQ. 1 ) THEN
          ! 'Wavenumber of the zero-upcrossing period'
          ! For an XSTO spectral tail
          NFIELD=1
          ...
          IF( SMCGRD ) THEN
#ifdef W3_SMC
            CALL W3S2XY_SMC( UXSP(1:NSEA,1), X1 )
#endif
          ELSE
            CALL W3S2XY ( NSEA, NSEA, NX+1, NY, UXSP(1:NSEA,1), MAPSF, X1 )
          ENDIF ! SMCGRD
!
       ...
       IF ( FLAGUNR ) CALL W3XYRTN(NSEA, UXSP(:,IK), UXSP(:,NZO+IK), AnglD)
            ...
            IF( SMCGRD ) THEN
#ifdef W3_SMC
         CALL W3S2XY_SMC( UXSP(:,IK), XX )
         CALL W3S2XY_SMC( UXSP(:,NZO+IK), XY )
#endif
            ELSE
              CALL W3S2XY ( NSEA, NSEA, NX+1,NY,UXSP(:,IK), MAPSF, XX )
              CALL W3S2XY ( NSEA, NSEA, NX+1,NY,UXSP(:,NZO+IK), MAPSF, XY )
            ENDIF ! SMCGRD
            XXK(:,:,IK-3)=XX
            XYK(:,:,IK-3)=XY
          END DO
!
        ELSE IF ( ISP .EQ. 3 ) THEN
          ...
          ! 'Stokes transport', but with the XSTO extended tail
          NFIELD=2
...
!
          ...
          IF( SMCGRD ) THEN
#ifdef W3_SMC
            CALL W3S2XY_SMC( UXSP(1:NSEA,2), XX )
            CALL W3S2XY_SMC( UXSP(1:NSEA,3), XY )
#endif
          ELSE
            CALL W3S2XY ( NSEA, NSEA, NX+1, NY, UXSP(1:NSEA,2), MAPSF, XX )
            CALL W3S2XY ( NSEA, NSEA, NX+1, NY, UXSP(1:NSEA,3), MAPSF, XY )
          ENDIF ! SMCGRD
          END IF ! ISP .EQ. 3
#endif
!
...
!
            ...
            ! CarstenHansen: ISUB > 0 is a sub-parameter index
            DO I=1,NFIELD
