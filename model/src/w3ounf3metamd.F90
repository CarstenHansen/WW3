...
!/! W3_MFIT! C Hansen note: FLG2D is for the fb_xsmf
    ...
    !
    !/ ------------------------------------------------------------------- /
!/! W3_MFIT!  NGRPP is for the fb_xsmf branch
...
#endif
!/! W3_MFIT! Add ('MFIT',7) to SPS,NSP
    ! These fields have sub-fields
    CHARACTER (LEN=6), DIMENSION(4) :: SPS = (/ 'P2S   ', 'BED   ', 'SVP   ', 'MFIT  ' /)
    ! Number of sub-fields for each element in SPS
    INTEGER, DIMENSION(4)           :: NSP = (/  2,        2,        3,        7 /)
    ! Sub-field unique integer id.'s
    INTEGER, DIMENSION(4) :: SPIJ2
    ...
    DO
      ...
      IF(TESTU == "META") THEN
        ...
        IF(IFC .LT. 1 .OR. IFC .GT. 3) THEN
          ...
          CALL EXTCDE(1)
        ENDIF
        IF(IFS .LT. 0 .OR. IFS .GT. NFSMAX) THEN
...
5015 FORMAT (/' *** WAVEWATCH III ERROR IN W3OUNFMETA : ' /          &
         ...
         '  => ', A /)
    !
5016 FORMAT (/' *** WAVEWATCH III ERROR IN W3OUNFMETA : ' /          &
         '     IFS value should be in range 1,',I2 /            &
         '     FILENAME = ', A /                                &
         '     LINE =', I5 /                                    &
         '  => ', A /)
    !
  ...
  !> @param[out] IFS    Sub-field number (defaults to 0)
  !>
  !> @author Chris Bunney @date 02-Feb-2021
  !/ ------------------------------------------------------------------- /
  SUBROUTINE DECODE_HEADER(BUF, ILINE, IFI, IFJ, IFC, IFS)
    ...
    !
    !     Both forms can also specify an optional component (IFC) integer
    !     value for multi-component fields (defaults to 1) and/or an optional
    !     integer (IFS) for a sub-field (defaults to 0).
    !
    !     Field name ID tags are case-insensitive, HS == hs == Hs.
    !
    !  3. Parameters :
...
2000 FORMAT ( '     FILENAME = ', A /                                &
  ...
  !> @param[in]  ISUB   Sub-field number (defaults to 0)
  !>
  !> @author Chris Bunney @date 02-Nov-2020
  !/ ------------------------------------------------------------------- /
  FUNCTION GETMETA(IFI, IFJ, ICOMP, IPART, ISUB) RESULT(META)
    ...
    !       ISUB    Int.  I  Sub-field number (defaults to 0 i.e. no sub-field)
    !     ----------------------------------------------------------------
    !
    !/ ------------------------------------------------------------------- /
    IMPLICIT NONE
...
1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3OUNFMETA : '           / &
    ...
    !
1003 FORMAT (/' *** WAVEWATCH III ERROR IN W3OUNFMETA : '           / &
         '     GETMETA: IFS value should be in range 0,',I2      / )
    !
...
1000 FORMAT (/' *** WAVEWATCH III ERROR IN W3OUNFMETA : '            / &
  ...
  SUBROUTINE DEFAULT_META(FLG2D)
    ...
    IF ( NCVARTYPE .EQ. 2 ) THEN
      ! This is if you specifically choose a short data type by setting
...
# ifdef W3_MFIT
    ELSE
      ! abs(xsm * 2 * ksc) is less than O(1)
      META(1)%FSC    = 1./30000.
      META(1)%VMIN = -1 
      META(1)%VMAX =  1
      META(1)%UNITS  = 'm/s'
      META(1)%VARNS=''
      META(1)%VARNM='MKX'
      META(1)%VARNL='Eastward wave pseudo-momentum * 2 * ksc'
      META(1)%VARNC=''
    END IF ! ( .NOT. FLG2D(IXMF,JXMF) )
# endif
    META(2)=META(1)
# ifdef W3_MFIT
    IF ( .NOT. FLG2D(IXMF,JXMF) ) THEN
...
# ifdef W3_MFIT
    ELSE
      META(2)%VARNM='MKY'
      META(2)%VARNL='Northward wave pseudo-momentum * 2 * ksc'
    END IF ! ( .NOT. FLG2D(IXMF,JXMF) )
# endif
#endif
    !
    ...
    META(1)%UNITS  = '1'
    META(1)%varnm = 'S0X'
    META(1)%varnl = 'Eastward surface Stokes-drift / abs(MK)'
    META(1)%varng=''
    META(1)%varns=''
    META(1)%VARND = DIRCOM
    META(1)%vmin = -32.766
    META(1)%vmax = 32.766
    META(1)%ENAME = '.s0'
    META(1)%varnc='U_S(Z) = Up(Z) + Ud(Z); '               &
                  // 'U_S(0) = S0 * (abs(MK)+0.01)'
    ! Second component
    META(2) = META(1)
    META(2)%varnm = 'S0Y'
    META(2)%varnl = 'Northward surface Stokes-drift / abs(MK)'
    META(2)%varnc='Up(Z) = Up(0) exp( -AM (Z ksc)^PM )'
    ! IFI=IXMF, IFJ=JXMF, ISP=2
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(2)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(1)%META(1)
    META(1)%varnm = 'SdX'
    META(1)%varnl = 'Eastward deep part Stokes-drift / abs(MK)'
    META(1)%ENAME = '.sd'
    META(1)%varnc='Ud(Z) = Ud(0) exp( -AMd (Z ksc)^PMd )'
    ! Second component
    META(2) = META(1)
    META(2)%varnm = 'SdY'
    META(2)%varnl = 'Northward deep part Stokes-drift / abs(MK)'
    META(2)%varnc = 'Ud(0) = Sd * abs(MK)'
    ! IFI=IXMF, IFJ=JXMF, ISP=3..7
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(3)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(1)%META(1)
    META(1)%VARND = ''
    META(1)%varnc = ''
    META(1)%vmin = 0
    META(1)%varnm = 'AM'
    META(1)%varnl = 'MFIT parameter AM'
    META(1)%ENAME = '.am'
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(4)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(3)%META(1)
    META(1)%varnm = 'PM'
    META(1)%varnl = 'MFIT parameter PM'
    META(1)%ENAME = '.pm'
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(5)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(3)%META(1)
    META(1)%varnm = 'AMd'
    META(1)%varnl = 'MFIT parameter AMd'
    META(1)%ENAME = '.amd'
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(6)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(3)%META(1)
    META(1)%varnm = 'PMd'
    META(1)%varnl = 'MFIT parameter PMd'
    META(1)%ENAME = '.pmd'
    META => GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(7)%META
    META(1) = GROUP(IXMF)%FIELD(JXMF)%SUBFIELD(3)%META(1)
    META(1)%varnm = 'SIC'
    META(1)%varnl = 'MFIT quality integer'
    META(1)%ENAME = '.sic'
    META(1)%FSC  = 1
    META(1)%vmin = -32766
    META(1)%vmax = 32766
...
#ifdef W3_MFIT
    NOGE(10) = NOGE(10)+1 ! This is probably not needed anymore
#endif      
    !
