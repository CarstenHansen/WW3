...
!/    15-Jan-2021 : Stokes drift with extended tail     ( version 7.XX )
!/                  ( C. Hansen )
!/    22-Mar-2021 : Add extra coupling fields as output ( version 7.13 )
!/    26-Jan-2022 : Stokes drift with extended tail     ( version 7.XX )
!/                  ( C. Hansen )
!/
...
!
      ...
      IF ( XSND .LE. 0 ) THEN
        ...
        END IF
# ifdef W3_MFIT
        FLGRD(10,NOEXTR+1) = .FALSE.
        FLGR2(10,NOEXTR+1) = .FALSE.
# endif
...
!
      ...
      CASE('XSP')
        I = IXSP
        J = JXSP
! Also w3ounfmeta GROUP(6)%FIELD(J)%SUBFIELD(1:3)%META used by CASE('XSP')
#endif
!
! Group 7
!
...
!
      ...
      CASE('MFIT')
        I = 10
        J = NOEXTR+1 ! Also w3ounfmeta GROUP(10)%FIELD(3)%SUBFIELD(1:7)
#endif
! Not found:
#ifdef W3_COU
      CASE('DRY')
...
!
#ifdef W3_XSTO
!  UXSP(JSEA,1:3+2*NZO) is set by CALC_XSTOKES(A), where NZO is either 0
!  (no Stokes profile output), or NZO = XSND (for full profile).
!  CALC_XSTOKES(A) has its own loop over JSEA and thus called after
!  the present loop
!  For profile fit under switch W3_MFIT, XSMH(JSEA,1:7) are set by CALC_MFIT()
#endif
          END IF
        END DO
!
...
# ifdef W3_MFIT
       IF ( FLOLOC(10,NOEXTR+1) ) THEN
         IF ( .NOT. FLOLOC(IXSP,JXSP) ) CALL CALC_XSTOKES(A)
         CALL CALC_MFIT()
       END IF
# endif
!        
...
!/
      ...
      USE W3ODATMD, ONLY: NOXSM, NOEXTR      
#endif
!/
...
#endif
#ifdef W3_MFIT
                IF ( FLOGRD( 10, NOEXTR+1) .AND. .NOT. FLOGRD( IXSP, JXSP) )  &
                                      UXSP(ISEA,:) = UNDEF
#endif
!
                IF ( FLOGRD( 7, 1) ) THEN
...
#ifdef W3_MFIT
                    IF ( IFJ .EQ. NOEXTR+1 ) THEN
                      WRITE ( NDSOG ) XSMH(1:NSEA,1:NOXSM)
                      IF ( .NOT. FLOGRD(IXSP,JXSP) ) WRITE ( NDSOG ) UXSP(1:NSEA,1:5)
                      ! Only the surface U_S and integral variables Kz,M
                      ! We presume that UXSP(:,3+2) point to XUXSP(:,5)
                    ELSE
