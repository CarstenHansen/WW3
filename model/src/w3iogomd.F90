...
!
    ...
    DO JSEA=1, NSEAL
      ...
      IF ( MAPSTA(IY,IX) .GT. 0 ) THEN
        ...
        !
        !  For profile fit under switch W3_MFIT, XSMH(JSEA,1:7) are set
        ! by CALC_MFIT()
      END IF
    END DO
    !
...
#endif
#ifdef W3_MFIT
          IF ( FLOGRD( 10, NOEXTR+1) .AND. .NOT. FLOGRD( ISVP, JSVP) )  &
            USVP(ISEA,:) = UNDEF
#endif
          !
          IF ( FLOGRD( 7, 1) ) THEN
...
# endif
            ...
            ELSE IF ( IFI .EQ. 10 ) THEN
              ...
              IF ( IFJ .EQ. NOEXTR+1 ) THEN
                ...
                IF ( .NOT. FLOGRD(ISVP,JSVP) ) WRITE ( NDSOG ) USVP(1:NSEA,1:5)
                ! Only the surface U_S and integral variables ksc,M
                ! We presume that USVP(:,3+2) point to XUSVP(:,5)
              END IF
              IF ( IFJ .LE. NOEXTR ) &
#endif
...
#ifdef W3_STVP
            ELSE IF ( IFI .EQ. ISVP .AND. IFJ .EQ. JSVP ) THEN
              READ (NDSOG,IOSTAT=IERR) USVP(1:NSEA,1:3+SPND*2)
              IF (IERR.NE.0) CALL EXTIOF(NDSE,IERR,'W3IOGO','',42)
#endif
              !
              !     Section 7)
...
#ifdef W3_MFIT
              IF ( IFJ .EQ. NOEXTR+1 ) THEN
                READ ( NDSOG,IOSTAT=IERR) XSMH(1:NSEA,1:NOXSM)
                IF ( .NOT. FLOGRD(ISVP,JSVP) )                 &
                  ! Only the surface U_S and integral variables ksc,M
                  READ (NDSOG,IOSTAT=IERR)USVP(1:NSEA,1:5)
              END IF
              IF ( IFJ .LE. NOEXTR ) &
#endif
