...
# Python2-function, replaced by build-in zip: from itertools import izip
...
for key in fm.variables.keys():
    ...
    if key==lon or key==lat or key=='time':
        ...
        dims=fm.variables[key].get_dims()
        # if dims: dims=dims[:]
        if not ( len(dims) == 2 ): continue
        dimn = [ dims[0].name, dims[1].name ]
        print('Consider variable',key+'(',dims,')')
        if  not ( lon in dimn and lat in dimn):
            print('Not a lon,lat field: ',key+'(',dimn,')')
            continue
...
vals_perline = int( 78/( len(format%0.0) + 1 )) # Presume format is a float '%NN.Nf'

print('assert ',var_shape,' == (',latlen,', ',lonlen,'); vals_perline == ',vals_perline)
...
for lati in range(latlen):
    ...
    for loni,lonj in zip(lon_linerange, lon_endrange):
